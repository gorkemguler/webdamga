...
async def capture(url: str, data_dir: Path, settings: CaptureSettings | None = None) -> dict:
    ...
    try:
        async with async_playwright() as pw:
            ...
            page.on(
                "console",
                lambda msg: console_lines.append(f"[{_now_iso()}] {msg.type.upper():8} {msg.text}"),
            )
            page.on(
                "pageerror",
                lambda exc: page_errors.append(f"[{_now_iso()}] {getattr(exc, 'message', exc)}"),
            )

            main_response: Response | None = None
            try:
                main_response = await page.goto(
                    url, wait_until=settings.wait_until, timeout=settings.timeout_ms
                )
            ...
            try:
                ...
                errors.append(f"screenshot: {str(exc).splitlines()[0]}")
            try:
            ...
            if main_response is not None:
                ...
                try:
                    ...
                    errors.append(f"response-meta: {str(exc).splitlines()[0]}")
            else:
            ...
            meta["console_message_count"] = len(console_lines)
    ...
    if page_errors:
        log_lines += ["", "=== PAGE ERRORS ===", *page_errors]
    (out_dir / "console.log").write_text("\n".join(log_lines) + ("\n" if log_lines else ""), encoding="utf-8")

    # --- sonlandırma ---
    meta["resource_summary"] = _summarize_har(har_path)
